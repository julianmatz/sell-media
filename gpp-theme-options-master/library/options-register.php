...
register_setting(
	// $option_group
	sell_media_get_current_theme_id() . '_options',
	// $option_name
	sell_media_get_current_theme_id() . '_options',
	// $sanitize_callback
	'sell_media_options_validate'
);
...
function sell_media_register_admin_js_globals(){
	...
	$selected_tab = $selected_tab ? $sell_media_tabs[0]['name'] : $tab;
...
add_action( 'admin_enqueue_scripts', 'sell_media_register_admin_js_globals', 1 );
 ...
 *
 */
function sell_media_options_validate( $input ) {

	global $sell_media_tabs;

	// This is the "whitelist": current settings
	$valid_input = (array) sell_media_get_options();
	// Get the array of Theme settings, by Settings Page tab
	$settingsbytab = sell_media_get_settings_by_tab();
	...
	foreach ( $tabsettings as $setting ) {
		...
		if ( 'submit' == $submittype ) {
			...
			$optiondetails = $sell_media_option_parameters[ $setting ];
 ...
 */
if ( ! empty( $sell_media_tabs ) ) {
foreach ( $sell_media_tabs as $tab ) {
	$tabname = $tab['name'];
	$tabsections = $tab['sections'];
	foreach ( $tabsections as $section ) {

		add_settings_section( 'sell_media_' . $section['name'] . '_section', $section['title'], 'sell_media_sections_callback', 'sell_media_' . $tabname . '_tab' );
 ...
 */
function sell_media_sections_callback( $section_passed ) {
	global $sell_media_tabs;
	foreach ( $sell_media_tabs as $tabname => $tab ) {
		$tabsections = $tab['sections'];
 ...
 */
global $sell_media_option_parameters;
$sell_media_option_parameters = sell_media_get_sell_media_option_parameters();
...
function sell_media_setting_callback( $option ) {
	...
	$fieldname = sell_media_get_current_theme_id() . "_options[ { $optionname } ]";

	$attr = $sell_media_option_parameters[ $option['name'] ];
    ...
    switch ( $fieldtype ) {
        ...
        //Render Text Input
        case 'text': sell_media_field_text( $value, $attr );
		echo '<span class="option-description">' . $option['description'] . '</span>';
        break;

        //Render textarea options
        case 'textarea': sell_media_field_textarea( $value, $attr );
		echo '<span class="option-description">' . $option['description'] . '</span>';
        break;

        //Render select dropdowns
        case 'select': sell_media_field_select( $value, $attr );
		echo '<span class="option-description">' . $option['description'] . '</span>';
        break;

        //Render radio dropdowns
        case 'radio': sell_media_field_radio( $value, $attr );
		echo '<span class="option-description">' . $option['description'] . '</span>';
        break;

        //Render radio image dropdowns
        case 'radio_image': sell_media_field_radio_image( $value, $attr );
		echo '<span class="option-description">' . $option['description'] . '</span>';
        break;

        //Render checkboxes
        case 'checkbox': sell_media_field_checkbox( $value, $attr );
		echo '<span class="option-description">' . $option['description'] . '</span>';
        break;

        //Render color picker
        case 'color': sell_media_field_color( $value, $attr );
		echo '<span class="option-description">' . $option['description'] . '</span>';
        break;

        //Render uploaded image
        case 'image': sell_media_field_image( $value, $attr );
		echo '<span class="option-description">' . $option['description'] . '</span>';
        break;

         //Render uploaded gallery
        case 'gallery': sell_media_field_gallery( $value, $attr );
		echo '<span class="option-description">' . $option['description'] . '</span>';
        break;
