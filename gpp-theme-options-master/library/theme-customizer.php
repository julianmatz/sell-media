...
function sell_media_customize_register( $wp_customize ) {
	...
	$sell_media_sell_media_option_parameters = sell_media_get_sell_media_option_parameters();

	foreach( $sell_media_sell_media_option_parameters as $option ) {

		$optionname = $option['name'];
		$optiondb = sell_media_get_current_theme_id() . "_options[$optionname]";
...
function sell_media_extract_valid_options( $options ) {
	$new_options = array();
...
function sell_media_customize_preview_js() {
	$doc_ready_script = '
		<script type="text/javascript">
			(function($){
				...
				wp.customize( "' . sell_media_get_current_theme_id() . '_options[logo]", function( value ) {
					value.bind( function( to ) {
						$( ".site-title a" ).html( "<img class=\"sitetitle\" alt=\"' . get_bloginfo( 'name' ) . '\" src=\"" + to + "\">" );
					});
				});

				wp.customize("' . sell_media_get_current_theme_id() . '_options[font]",function(value) {
					value.bind(function(to) {
						$("#fontdiv").remove();
				...
				wp.customize("' . sell_media_get_current_theme_id() . '_options[font_alt]",function(value) {
					value.bind(function(to) {
						$("#fontaltdiv").remove();
						var googlefont = to.split(",");
						var n = googlefont[0].indexOf(":");
				...
				wp.customize( "' . sell_media_get_current_theme_id() . '_options[color]", function( value ) {
					value.bind( function( to ) {
						$( "#gpp-alt-style-css" ).attr( "href", "' . get_template_directory_uri() . '/css/" + to + ".css" );
					});
				});

				wp.customize( "' . sell_media_get_current_theme_id() . '_options[css]", function( value ) {
					value.bind( function( to ) {
						$( "#tempcss" ).remove();
						var googlefont = to.split( "," );
